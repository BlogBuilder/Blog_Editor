            ...
            <div class="portlet-body">
                ...
                <form action="#" class="form-horizontal" id="article_add">
                    <div class="form-body">
                        ...
                        <div class="form-group form-md-line-input" v-show="type!=5">
        ...
        methods: {
            ...
            _loadData(){
                ...
                me.$http.get("/api/article/findById", {
                    ...
                    me.title = data.title;
                    me.categoryId = data.category.id;
                    me.type = data.type;
                    me.summary = data.summary;
                    me.quote_author = data.quote_author;
                    editor.txt.html(data.content);


                    for (var j = 0; j < data.tag.length; j++) {
                        me.tags.push(data.tag[j].id);
                    }

                    if (data.type == 1) {
                        for (var i = 0; i < data.materials.length; i++) {
                            me.resource.push(data.materials[i].material);
                        }
                    } else {
                        if (data.type != 5) {
                            me.resource.push(data.materials.material);
                        }
                    }

                    $('#categoryId').selectpicker("val", me.categoryId);
                    $('#tags').selectpicker("val", me.tags);
                }, () => {
                    serverErrorInfo();
                })
            },

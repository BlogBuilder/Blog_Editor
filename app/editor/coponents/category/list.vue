...
<script type="es6">
    ...
    module.exports = {
        ...
        methods: {
            ...
            fetchData (pageNum) {
                const me = this;
                me.$http.get('/api/category/adminList', {
                    params: {
                        rowCount: rowCount,
                        currentPage: pageNum,
            ...
            search(event){
                ...
                if (event.target.value) {
                    me.$router.push(redictURL(me.$route.fullPath, "/category/list", "add", "key", event.target.value));
                } else {
                    me.$router.push(redictURL(me.$route.fullPath, "/category/list", "remove", "key"));
                }
            },
            create(){
                const me = this;
                me.$router.push("/category/create");
            },
            edit(item){
                const me = this;
                me.$router.push("/category/change?id=" + item.id);
            },
            remove(id){
                var me = this;
                confirm({
                    content: "是否删除当前分类信息？",
                    success: function () {
                        me.$http.get("/api/category/delete", {
                            params: {
                                id: id
                            }
                        }).then(response => {
            ...
            removeAll(){
                ...
                confirm({
                    content: "是否删除当前选中文章分类？",
                    success: function () {
                        me.$http.get("/api/category/deleteAll", {
                            params: {
